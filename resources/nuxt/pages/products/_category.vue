<template>
  <section>
    <v-container v-if="category" class="pa-7">
      <v-card color="pink darken-3" rounded class="mt-4">
        <v-row justify="center" align-content="center">
          <v-col cols="12" md="3">
            <v-img max-height="140px" :src="'/storage/' + category.image"></v-img>
          </v-col>
          <v-col cols="12" md="9">
            <h1 class="white--text display-2 text-center" style="margin-top: 40px">
              {{ category.name }}
            </h1>
          </v-col>
        </v-row>
    ...
    <v-container>
      ...
      <v-row class="mt-6">
        <v-col :md="grid ? '3' : '12'" sm="6" v-for="product in products" cols="12">
...
export default {
  ...
  methods: {
    ...
    getCategory() {
      ...
      this.$apollo
        .query({
          query: require('~/apollo/queries/category'),
          variables: {
            id
          }
        })
